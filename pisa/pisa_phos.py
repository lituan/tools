...
def select_phos_site(pdb_interfaces):
    ...
    for interface in pdb_interfaces:
        ...
        for phos,interacting_residues in phos_interacting_residues.items():
            ...
            interacting_chain = [c for c in chains if c[0] == interacting_residues[0].split('_')[0]][0]
...
def main():
    ...
    phos_chain_length = [p[-2][3] for p in pdb_phos_sites]
    sns.distplot(phos_chain_length)
    plt.savefig(fname+'_phos_chain_length_dist.png',dpi=300)

    pdb_phos_sites = [p for p in pdb_phos_sites if p[-2][3] < 20]
