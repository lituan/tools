...
from plip_phos_net import myplip
...
def check_current_directory(directory):
    ...
    for root,dirs,files in os.walk(direcotry):
        for f in files:
            if f[-4:] == '.png' and len(f.split('_')) == 4:
                current_got_pdbids.append(f[:4].upper())
    return current_got_pdbids
...
def main():
    ...
    current_got_pdbis = check_current_directory('.')
    ...
    for phosi,polyi,pdbids in r:
        if pdbids:
            ...
            if pdbids:
                pdbids = [p for p in pdbids if not p in current_got_pdbis]
                if pdbids:
                    pdbids = ','.join(pdbids)
                    parameters.append((pdbids,phosi+'_'+polyi))

    p = Pool(4)
    r = p.map(myplip,parameters)
    p.close()
